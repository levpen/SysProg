...
void *_thread_worker(void *arg_pool)
{
	...
	while (1)
	{
		...
		else
		{
			...
			// printf("!!!worker locking status 2!!!\n");

			pthread_mutex_lock(&cur_task->mutex_status);
			// printf("worker locked status\n");

			pthread_mutex_lock(&cur_task->mutex_detached);
			if(cur_task->pool == NULL) {
				pthread_mutex_unlock(&cur_task->mutex_status);
				// printf("deleting previous task after finished task\n");
				__atomic_sub_fetch(&pool->tasks_count, 1, __ATOMIC_ACQ_REL);
				pthread_mutex_unlock(&cur_task->mutex_detached);
				thread_task_delete(cur_task);
				continue;
			...
			pthread_mutex_unlock(&cur_task->mutex_status);
			// printf("!!!worker unlocked status 2!!!\n");
		}
	}
	return NULL;
}
...
int thread_pool_push_task(struct thread_pool *pool, struct thread_task *task)
{
	...
	task->pool = pool;
	task->result = NULL;
	__atomic_store_n(&task->status, T_CREATED, __ATOMIC_RELAXED);
